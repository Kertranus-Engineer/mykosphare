...
import { ingestEnvironmentalEvent } from "@/lib/ingestion"
import { validateEnvironmentalEvent } from "@/lib/ingestion"
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const validation = validateEnvironmentalEvent(body)
  if (!validation.valid) {
  ...
  const result = await ingestEnvironmentalEvent(validation.data!)
