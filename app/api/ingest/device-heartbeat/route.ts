...
import { ingestDeviceHeartbeat } from "@/lib/ingestion"
import { validateDeviceHeartbeat } from "@/lib/ingestion"
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const validation = validateDeviceHeartbeat(body)
  if (!validation.valid) {
  ...
  const result = await ingestDeviceHeartbeat(validation.data!)
